...
dd.getWndW = function() {
    return dd.Int(
        (dd.db && !dd.op && !dd.w3c && dd.db.clientWidth)
        ? dd.db.clientWidth
        : (window.innerWidth || 0)
    );
};
dd.getWndH = function() {
    return dd.Int(
        (dd.db && !dd.op && !dd.w3c && dd.db.clientHeight)
        ? dd.db.clientHeight
...
dd.getDivW = function(d_o) {
    return dd.Int(
        dd.n4
        ? (d_o.div ? d_o.div.clip.width : 0)
        : d_o.div
        ? (d_o.div.offsetWidth || d_o.css.pixelWidth || d_o.css.width || 0)
        : 0
    );
};
dd.getDivH = function(d_o) {
    return dd.Int(
        dd.n4
        ? (d_o.div ? d_o.div.clip.height : 0)
        : d_o.div
        ? (d_o.div.offsetHeight || d_o.css.pixelHeight || d_o.css.height || 0)
        : 0
    );
};
...
dd.getParent = function(d_o, d_p) {
    if (dd.n4) {
        for (var d_i = dd.elements.length; d_i;) {
            if (!((d_p = dd.elements[--d_i]).is_image) &&
                d_p.div &&
                (d_p.div.document.layers[d_o.name] || d_o.oimg && d_p.div.document.images[d_o.oimg.name]))
...
dd.getCmdVal = function(d_o, d_cmd, d_cmdStr, int0) {
    var d_i = d_o.id.indexOf(d_cmd),
        d_j,
        d_y = (d_o.id.indexOf(d_cmd) >= 0)
            ? dd.Int(d_o.id.substring(d_o.id.indexOf(d_cmd) + d_cmd.length))
            : int0
            ? -1
            : 0;
...
dd.evt = function(d_e) {
    ...
    this.button = (this.e.type == "mousedown")
        ? this.but
        : (dd.e && dd.e.button)
        ? dd.e.button
        : 0;
        ...
        if (!(d_o = dd.elements[--d_i]).is_image && d_o.div) {
            ...
            if (d_o.div.pos_rel) {
                ...
                for (var d_p, d_j = d_o.children.length; d_j;) {
                    if (!(d_p = d_o.children[--d_j]).detached &&
                        (d_o != d_p.defparent || !(d_p.is_image && dd.getImg(d_p, d_p.oimg.name, 1)))) {
                        d_p.defx += d_dx;
                        d_p.defy += d_dy;
            ...
            if (!(d_o.parent && d_o.parent != d_o.defparent) &&
                (d_x || !d_o.detached || d_o.horizontal || d_o.vertical) &&
                dd.getImg(d_o, d_o.oimg.name, 1))
...
function DDObj(d_o, d_i) {
    ...
    if (this.oimg) {
        ...
        this.t_htm = '<div id="' +
            ...
            "left:" +
            (this.cssx = this.x = this.defx) +
            "px;" +
            "top:" +
            (this.cssy = this.y = this.defy) +
            "px;" +
            "width:" +
            this.w +
            "px;" +
...
DDObj.prototype.copy = function(d_n, d_p, d_b) {
    if (!this.oimg) return;
    d_b = (dd.ie && document.all.tags("body"))
        ? document.all.tags("body")[0]
        : document.getElementsByTagName
        ? (document.getElementsByTagName("body")[0] || dd.db)
        : dd.db;
...
dd.n4Rect = function(d_w, d_h) {
    ...
    for (d_i = 4; d_i;) {
        --d_i;
        dd.n4RectPos(dd.rectI[d_i],
            dd.obj.x + (!(d_i - 1) ? (dd.obj.w - 1) : 0),
            dd.obj.y + (!(d_i - 2) ? (dd.obj.h - 1) : 0),
            d_i & 1 || dd.obj.w,
            !(d_i & 1) || dd.obj.h);
        dd.n4RectPos(dd.rectA[d_i],
            !(d_i - 1) ? dd.rectI[1].x + 1 : (dd.obj.x - 1),
            !(d_i - 2) ? dd.rectI[2].y + 1 : (dd.obj.y - 1),
            d_i & 1 || dd.obj.w + 2,
            !(d_i & 1) || dd.obj.h + 2);
    }
};
...
dd.embedVis = function(d_vis) {
    ...
    for (var d_i = d_o.length; d_i;) {
        --d_i;
        var d_p = dd.ie
            ? document.all.tags(d_o[d_i])
            : document.getElementsByTagName
            ? document.getElementsByTagName(d_o[d_i])
            : null;
        if (d_p) {
...
dd.maxOffX = function(d_x, d_y) {
    return(
        (dd.obj.maxoffl + 1 && (d_y = dd.obj.defx - dd.obj.maxoffl) - d_x > 0 ||
                dd.obj.maxoffr + 1 && (d_y = dd.obj.defx + dd.obj.maxoffr) - d_x < 0)
            ? d_y
            : d_x
    );
};
dd.maxOffY = function(d_x, d_y) {
    return(
        (dd.obj.maxofft + 1 && (d_y = dd.obj.defy - dd.obj.maxofft) - d_x > 0 ||
                dd.obj.maxoffb + 1 && (d_y = dd.obj.defy + dd.obj.maxoffb) - d_x < 0)
            ? d_y
            : d_x
    );
};
dd.inWndW = function(d_x, d_y) {
    var d_wx = dd.getScrollX(),
        d_ww = dd.getWndW();
    return(
        ((d_y = d_wx + 2) - d_x > 0) || ((d_y = d_wx + d_ww + dd.obj.w - 2) - d_x < 0)
            ? d_y
            : d_x
    );
};
dd.inWndH = function(d_x, d_y) {
    var d_wy = dd.getScrollY(),
        d_wh = dd.getWndH();
    return(
        ((d_y = d_wy + 2) - d_x > 0) || ((d_y = d_wy + d_wh + dd.obj.h - 2) - d_x < 0)
            ? d_y
            : d_x
    );
};
...
dd.limW = function(d_w) {
    return(
        (dd.obj.minw - d_w > 0)
            ? dd.obj.minw
            : (dd.obj.maxw > 0 && dd.obj.maxw - d_w < 0)
            ? dd.obj.maxw
            : d_w
    );
};
dd.limH = function(d_h) {
    return(
        (dd.obj.minh - d_h > 0)
            ? dd.obj.minh
            : (dd.obj.maxh > 0 && dd.obj.maxh - d_h < 0)
            ? dd.obj.maxh
            : d_h
    );
};
...
function DDScroll() {
    ...
    if (dd.msmoved) {
        ...
        dd.scrx = ((d_y = dd.e.x - d_ww - d_wx + d_bnd) > 0)
            ? (d_y >>= 2) * d_y
            : ((d_y = d_wx + d_bnd - dd.e.x) > 0)
            ? -(d_y >>= 2) * d_y
            : 0;
        dd.scry = ((d_y = dd.e.y - d_wh - d_wy + d_bnd) > 0)
            ? (d_y >>= 2) * d_y
            : ((d_y = d_wy + d_bnd - dd.e.y) > 0)
            ? -(d_y >>= 2) * d_y
            : 0;
    }
    if (dd.scrx || dd.scry) {
        window.scrollTo(
            d_wx +
            (dd.scrx = dd.obj.is_resized
                ? dd.limW(dd.obj.w + dd.scrx) - dd.obj.w
                : dd.obj.vertical
                ? 0
                : (dd.maxOffX(dd.obj.x + dd.scrx) - dd.obj.x)),
            d_wy +
            (dd.scry = dd.obj.is_resized
                ? dd.limH(dd.obj.h + dd.scry) - dd.obj.h
                : dd.obj.horizontal
                ? 0
                : (dd.maxOffY(dd.obj.y + dd.scry) - dd.obj.y))
        );
        dd.obj.is_dragged
            ? dd.obj.moveTo(dd.obj.x + dd.getScrollX() - d_wx, dd.obj.y + dd.getScrollY() - d_wy)
...
function PICK(d_ev) {
    ...
    for (var d_o, d_t, d_cmp = -1, d_i = dd.elements.length; d_i;) {
        ...
        if (d_o.visible &&
            ...
            dd.e.y <= d_o.y + d_o.h) {
            if (d_o.z > d_cmp &&
                (d_t = dd.e.src.tag).indexOf("inpu") < 0 &&
                d_t.indexOf("texta") < 0 &&
                d_t.indexOf("sele") < 0 &&
                d_t.indexOf("opti") < 0 &&
                d_t.indexOf("scrol") < 0) {
    ...
    if (dd.n4 || dd.n6 || dd.ie || dd.op || dd.w3c)
        document.write(
            (dd.n4
                ...
                : (dd.op && !dd.op6)
                ? '<div id="OpBlUr" style="position:absolute;visibility:hidden;width:0px;height:0px;"><form><input type="text" style="width:0px;height:0px;"><\/form><\/div>'
                : "") +
            d_htm
        );
    dd.z = 0x33;
