...
namespace Loom.Data.Mapping.Query
{
    public class DbTextWriter : StringWriter
    {
        ...
        protected virtual string UniquePrefix => Formats.Prefix;

        protected virtual string ColumnSeparator => Formats.Seperator;

        protected bool IsJoinStarted => flags[IsJoinStartedFlag];

        protected bool IsWhereStarted
        {
            get => flags[IsWhereStartedFlag];
            private set => flags[IsWhereStartedFlag] = value;
        }

        protected bool IsHavingStarted
        {
            get => flags[IsHavingStartedFlag];
            private set => flags[IsHavingStartedFlag] = value;
        }

        protected bool IsFromStarted
        {
            get => flags[IsFromStartedFlag];
            private set => flags[IsFromStartedFlag] = value;
        }

        protected internal bool IsSelectionSet
        {
            get => flags[IsSelectionSetFlag];
        ...
        public virtual void WriteFrom(IEnumerable<ISchema> tables, bool useTableAlias = false)
        {
            if (!IsSelectionSet)
            {
                Write(Formats.SelectAllColumns);
            ...
            foreach (ISchema table in tables)
            {
                if (IsFromStarted)
                    Write(", ");
        ...
        {
            Write(Formats.AndOperator);
        }

        public virtual void WriteOr()
        {
            Write(Formats.OrOperator);
        }

        internal string GetTableAlias(ISchema table)
        {
        ...
        /// <remarks>
        ///     <para>
        ///         The default format is [Owner].[Table] Alias
        ///     </para>
        /// </remarks>
        ...
        protected virtual string FormatTableName(ISchema table, bool needsAlias)
        {
