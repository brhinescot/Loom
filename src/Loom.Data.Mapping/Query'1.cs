...
namespace Loom.Data.Mapping
{
    ...
    {
        internal EntitySet(DataSession session) : base(session, DataRecord<T>.Table) { }
        ...
        ///         <see cref="DataSet" /> returned from <see cref="CommandBase{T}.FetchDataSet" />
        ///     </para>
        ///     <para>
        ///         Use <see cref="IDataReader.NextResult" /> to access the result of the batched queries.
        ///     </para>
        /// </remarks>
        ...
        ///     The <see cref="Query.ColumnPredicate" /> can not be converted
        ///     to a <see cref="JoinPredicate" />. The right side object must be an
        ///     <see cref="IQueryableColumn" /> instance.
        /// </exception>
        ...
        {
            Argument.Assert.IsNotNull(fromColumn, nameof(fromColumn));
            Argument.Assert.IsNotNull(toColumn, nameof(toColumn));
        ...
        private void AutoJoin(ITable toTable)
        {
...
//                throw new InvalidOperationException(string.Format("Unable to map table {0}.{1} to table {2}.{3}.", QueryTree.Table.Owner, QueryTree.Table.Name, toTable.Owner, toTable.Name));
        ...
        public EntitySet<T> OrderBy(IQueryableColumn column1, IQueryableColumn column2, IQueryableColumn column3)
        {
            Argument.Assert.IsNotNull(column1, nameof(column1));
            Argument.Assert.IsNotNull(column2, nameof(column2));
            Argument.Assert.IsNotNull(column3, nameof(column3));
        ...
        ///     The number of rows after the <paramref name="startRowIndex" />
        ///     to return.
        /// </param>
        /// <returns>
        ///     An instance of the <see cref="EntitySet{T}" /> class used to refine and execute
        ///     the query.
        /// </returns>
